...
{
    [Bot("BreakBBReverse")]
    public class BreakBBReverse : BotPanel
    {
        BotTabSimple _tab;
        ...
        private Aindicator _bbc_short;
        private StrategyParameterInt _length_short;
        private StrategyParameterString _TwoPoint_short;

        private Aindicator _sma_long;
        private StrategyParameterInt _periodSma_long;

        private Aindicator _sma_short;
        private StrategyParameterInt _periodSma_short;
        ...
        private void StopOrActivateIndicators()
        {
            ...
            {
                _bbc_long.IsOn = true;
                _bbc_long.DataSeries[0].IsPaint = true;
                _bbc_long.DataSeries[1].IsPaint = true;
                _sma_long.IsOn = true;
                _sma_long.DataSeries[0].IsPaint = true;

                _bbc_short.IsOn = false;
                _bbc_short.DataSeries[0].IsPaint = false;
                _bbc_short.DataSeries[1].IsPaint = false;
                _sma_short.IsOn = false;
                _sma_short.DataSeries[0].IsPaint = false;
            }
            else if (Regime.ValueString == "OnlyShort")
            {
                _bbc_short.IsOn = true;
                _bbc_short.DataSeries[0].IsPaint = true;
                _bbc_short.DataSeries[1].IsPaint = true;
                _sma_short.IsOn = true;
                _sma_short.DataSeries[0].IsPaint = true;

                _bbc_long.IsOn = false;
                _bbc_long.DataSeries[0].IsPaint = false;
                _bbc_long.DataSeries[1].IsPaint = false;
                _sma_long.IsOn = false;
                _sma_long.DataSeries[0].IsPaint = false;
            }
            else
            {
                _bbc_short.IsOn = true;
            ...
            {// enter logic
                        ...
                        && !have_long_pos)
                {
                    if (!BuySignalIsFiltered(candles))
                    {
                        ...
                        {
                            _slippage = Slippage.ValueDecimal * bb_down_long / 100;
                            _tab.BuyAtStop(GetVolume(), bb_down_long + _slippage, bb_down_long, StopActivateType.LowerOrEqual, 1);
                        }
                        else
                        {
                        ...
                        && !have_short_pos)
                {
                    if (!SellSignalIsFiltered(candles))
                    {
                        if (ReverseLogic.ValueBool)
                        {
                            _slippage = Slippage.ValueDecimal * bb_up_short / 100;
                            _tab.SellAtStop(GetVolume(), bb_up_short - _slippage, bb_up_short, StopActivateType.HigherOrEqual, 1);
                        }
                        else
                        {
                            _slippage = Slippage.ValueDecimal * bb_down_short / 100;
                            _tab.SellAtStop(GetVolume(), bb_down_short - _slippage, bb_down_short, StopActivateType.LowerOrEqual, 1);
                        }
                    }
                }
